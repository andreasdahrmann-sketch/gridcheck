...
# 3. Spannung-N-1
# ----------------------------------------------------------------------
def bewerte_spannung_n1(spannung_n1: dict | None) -> dict:
    """
    Spannung-N-1: ΔU bei Ausfall eines Parallelsystems.

    Eingabe: {"delta_u_prozent": float} aus szenarien-Berechnung für N-1.
    Grenzen: GRÜN ≤ 5%, GELB ≤ 10% (DIN EN 50160 / VDE-AR-N 4110).
    """
    if not spannung_n1:
        return {
            "bewertung": "NICHT_GEPRUEFT",
            "delta_u_n1_prozent": None,
            "grenze_prozent": GRENZEN["spannung_n1"]["gelb_max_prozent"],
    ...
    bew = _bewertung_aus_prozent(
        delta_abs,
        GRENZEN["spannung_n1"]["gruen_max_prozent"],
        GRENZEN["spannung_n1"]["gelb_max_prozent"],
    )

    tech = (f"N-1 Spannung: |ΔU_N1|={delta_abs:.2f}% "
            f"(Grenzen: GRÜN≤{GRENZEN['spannung_n1']['gruen_max_prozent']}%, "
            f"GELB≤{GRENZEN['spannung_n1']['gelb_max_prozent']}% gem. DIN EN 50160 / VDE-AR-N 4110).")
    ...
    if bew == "GRUEN":
        ...
        klar = f"Spannungsänderung im Ausfallfall {delta_abs:.1f}% — überschreitet zulässige 10%. Spannungshaltung erforderlich."
...
def analysiere_n1(eingabe: dict,
    ...
    # 4. Spannung-N-1
    n1_spg = bewerte_spannung_n1(spannung_n1)
