...
def bewerte_n1_ms(eingabe: dict) -> dict:
    ...
    if topologie == "unbekannt":
        ...
        if not rest_bekannt:
            ...
            konfidenz = "hoch"
            begruendung_tech = (
                f"Topologie {topologie} grundsaetzlich N-1-faehig, aber "
                "Restkapazitaet unbekannt. Pre-Check kann N-1 nicht bestaetigen."
            )
            annahmen.append("Restkapazitaet MS unbekannt -> konservativ 0 MVA")
            empfehlungen.append("Restkapazitaet beim VNB anfragen")
        elif rest_mva + 1e-9 < s_mva:
            bewertung = "ROT"
            n1_sicher = False
            konfidenz = "hoch"
