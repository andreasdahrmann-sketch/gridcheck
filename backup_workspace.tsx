...
export default function ProjectDetailWorkspace({ projectId: projectIdParam }: { projectId: string }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: Parameters<typeof updateProject>[1]) => updateProject(projectIdNum, payload),
    onSuccess: () => {
  ...
  const shareMutation = useMutation({
    mutationFn: (userId: number) => shareProject(projectIdNum, userId, "viewer"),
    onSuccess: () => {
      setShareUserId("");
                  ...
                  {analysisOptions.length > 0 ? (
                    <div className="rounded-[22px] border border-white/10 bg-black/10 p-4">
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="mt-1 text-xs text-text-muted">
                                Scope {option.package_scope} Â· {getReportScopeLabel(option.report_scope)}
                              </p>
                            </button>
                          );
                        })}
                      ...
                      <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3 text-xs leading-5 text-text-muted">
                        {selectedPackageProfile.title} Â· {getPackageScopeLabel(selectedPackageScope)} Â·{" "}
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/10 p-4">
                    ...
                    <div className="mt-2 space-y-1 text-white">
                      ...
                      <p className="text-text-muted">
                        Risiken: {projectCostBand?.drivers?.length ? projectCostBand.drivers.join(" Â· ") : "Werden nach dem ersten Run verdichtet."}
                      </p>
                    </div>
                  </div>
                ) : null}
